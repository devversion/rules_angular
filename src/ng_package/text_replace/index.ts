...
async function applySubstitutions(origin: string, destination: string, substitutions: [RegExp, string][]) {
...
async function main(args: string[]) {
  const [substitutionsArg, originsRaw, destinationRaw] = (await fs.readFile(args[0], {encoding: 'utf-8'})).split('\n').map(line => line.replace(/^'(.*)'$/, '$1'));
  /** List of files to propcess for substitution */
  const origins = JSON.parse(originsRaw) as BazelFileInfo[];
  /** The destination directory for the copied files with substitutions applied. */
  const destinationDir = JSON.parse(destinationRaw) as BazelFileInfo;
  /** Map of substitutions to make whenever the regex key is matched in a file's content. */
  const substitutions = Object
    .entries(JSON.parse(substitutionsArg) as Record<string, string>)
    .map<[RegExp, string]>(([key, val]) => ([new RegExp(key, 'g'), val]));
  ...
  for (let origin of origins) {
    files.push(
      ...
      ...globSync("**", {cwd: origin.path})
        .map<[string, string]>(file => ([origin.path, file])
      )
    );
  }

  // Wait for substitutions to asynchronously occur on all files.
  await Promise.all(files.map(([origin, file]) => 
    applySubstitutions(path.join(origin, file), path.join(destinationDir.path, file), substitutions)
  ));
}
